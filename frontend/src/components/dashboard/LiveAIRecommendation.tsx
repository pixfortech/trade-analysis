...
export function LiveAIRecommendation() {
  ...
  return (
    ...
    >
      ...
      <InstrumentSearch onSelect={onSelect} placeholder="Instrument for recommendation…" />
      ...
      <div className="mt-4">
        ...
        {rec.data && <Recommendation s={rec.data} />}
      </div>
    </Card>
  );
}

function Recommendation({ s }: { s: LiveSignal }) {
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "bull" | "bear" }) {
  ...
  return (
    <div className="rounded-lg border border-white/5 bg-base-800/60 px-3 py-2">
      ...
      <p className={`num text-[15px] font-semibold ${c}`}>{value}</p>
    </div>
  );
}
